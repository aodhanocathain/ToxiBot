...
const {Piece, BlockablePiece, RangedPiece, King, Queen, Rook, Knight, Pawn, PieceClassesByTypeChar} = require("./Piece.js");
...
{	
	...
	updateMovingPieceAndPiecesSeeingMove(movingPiece, move)
	{
		...
		[this.movingTeam, this.movingTeam.opposition].forEach((team)=>{
			[move.mainBefore, move.mainAfter].forEach((square)=>{
				const squareSubscribers = team.subscribers[square].slice();
				//squareSubscribers.forEach(([piece, directionIndex])=>{
				squareSubscribers.forEach((thing)=>{
					if(!("length" in thing)){console.log("thing is not an array");}
					const [piece, directionIndex] = thing;
					if(piece.isActive())
					{
						///*
						if(piece instanceof RangedPiece)
						{
							piece.updateDirectionIndex(directionIndex);
							partUpdatedPieces.push([piece, directionIndex]);
						}
	...
	switchMoveBySamePiece(move)
	{
		...
		this.partUpdatedPieces.pop().forEach(([piece, directionIndex])=>{piece.revertDirectionIndex(directionIndex);});
	...
	kingCapturable()
	{	
		return this.movingTeam.subscribers[this.movingTeam.opposition.king.square].some(([piece, directionIndex])=>{return piece.isActive();});
	}
	
	kingChecked()
	{
		return this.movingTeam.opposition.subscribers[this.movingTeam.king.square].some(([piece, directionIndex])=>{return piece.isActive();});
	}
